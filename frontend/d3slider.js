...
(function($, d3, moment) {
  ...
  var dataset = function() {
    ...
    // Change stand and end date to change slider range
    var arr = [],
      startDate = moment("1960-01-01"),
      endDate = moment("2017-01-01");
    for (startDate; startDate < endDate; startDate.add(Math.ceil(Math.random() * 1) + 2, "days")) {
      arr.push({
  ...
  // *** Settings for the location and size of the slider *** //
  ...
  // Variable function to make the slider
  var renderSlider = function(dataset, settings, callback) {
    ...
    var RangeSlider = function(svg, width, radius, color, translater, callback) {
      ...
      // build the handles to select range

      // Left hand slider handle
      ...
      // Text hovering over the left slider handle
      ...
      // Text hovering over the right slider handle
    ...
    RangeSlider.prototype.init = function() {
      ...
      // Slider handles movement handling
      self.move = function(self) {
        ...
        // Movement of the left slider handle
        api.$min = function(x) {
        ...
        // Movement of the right slider handle
        api.$max = function(x) {
      ...
      // Events for starting to drag one of the handles
      ...
      // Events for stopping of dragging one of the handles
    ...
    // Creation of the svg to display the slider on
  ...
  var updateChart = function(dataset, colors, settings) {
    ...
    var callback = function(process, dstart, dend) {
      if (process === 'dragend') {          
        ...
        // Here the values for the the slider range is returned (currently log, but we can use this for the visualization with the correct return)
        console.log(dstart.value, dend.value);
      }
    };
